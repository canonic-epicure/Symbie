...
//    id : [ 'UID' ],
    ...
    my : {
        ...
        methods : {
            ...
            constructID : function (params) {
                var meta = this.targetMeta
                var id = meta.name
                
                var ids = []
                
                Joose.O.each(meta.idDefinition || {}, function (value, name) {
                    ids.push(name)
                })
                ...
                if (ids.length) {
                    ...
                    Joose.A.each(ids, function (name) {
                        id += ':' + params[name]
                    })
                }
    ...
    before : {
        initComponent : function () {
            this.UID = this.constructor.my.UID++
            
            Joose.O.each(this.meta.idDefinition || {}, function (value, name) {
                if (this[name] == null) throw "Required parameter [" + name + "] (part of [" + this.meta.name + "]'s id is not supplied"
            }, this)
            
            this.id = this.constructor.my.constructID(this)
        },
    ...
    methods : {
        ...
        prepareInstance : function (request, done) {
            done()
        }
    }
})
