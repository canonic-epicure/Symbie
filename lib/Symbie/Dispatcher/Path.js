Class('Symbie.Dispatcher.Path', {
    ...
    methods : {
        ...
        prepareClasses : function (callback, scope) {
            var depsInfo = []
            
            Joose.A.each(this.elements, function (element) {
                if (element instanceof Symbie.Dispatcher.PathElement.Widget) depsInfo.push(element.token)
            })
            
            use(depsInfo, callback, scope)
        }
